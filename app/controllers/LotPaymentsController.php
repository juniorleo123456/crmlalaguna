...
class LotPaymentsController extends Controller
{
    ...
    private function form(string $mode, int $id = 0)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $errors = [];
            if ($data['amount'] <= 0) $errors[] = 'El monto debe ser mayor a 0';
            if (empty($data['payment_type'])) $errors[] = 'Selecciona un tipo de pago';
            if (!$data['lot_sale_id'] && !$data['lot_reservation_id']) $errors[] = 'Asocia el pago a una venta o reserva';
