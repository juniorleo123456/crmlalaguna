...
class LotsController extends Controller
{
    ...
    private function form(string $mode, int $id = 0)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $errors = [];
            if ($data['block_id'] <= 0) $errors[] = 'Debes seleccionar una manzana válida';
            if (empty($data['lot_number'])) $errors[] = 'El número del lote es obligatorio';
            if ($data['area'] <= 0) $errors[] = 'El área debe ser mayor a 0';
